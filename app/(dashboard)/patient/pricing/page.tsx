...
const pricing = [
	...
	{
		title: "TIER 3",
		banner: { title: "recommended", className: tw`border border-medinfo-light-1 bg-white` },
	...
	return (
		<Main className="w-full gap-8 max-md:mx-auto max-md:max-w-[400px] md:gap-12">
			...
			<PricingCardList
				...
				each={pricing}
				render={(item) => <PricingCard key={item.title} {...item} />}
			/>
		</Main>
	);
}
			...
			<div className="flex flex-col gap-9">
				<Card.Header className="flex justify-between">
					...
					{prices && (
						...
						>
							<For each={prices} render={(item) => <span key={item}>{item}</span>} />
						</Card.Description>
					)}
				</Card.Header>
